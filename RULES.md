...
22. 如果用户要求发布：
    - 先检查 `fanqie_check_login` 工具是否可用（MCP 是否已加载）
    - 如果 MCP 工具不可用：检查 `mcp/fanqie/dist/index.js` 是否存在
      - 不存在 → 从本仓库复制 `mcp/fanqie/` 并运行 `npm install && npm run build`
      - 存在但未配置 → 创建项目级 MCP 配置文件（`.claude/mcp.json`），提示用户重启会话
    - 使用 `fanqie_check_login` 检查登录状态（MCP 会自动启动 Chrome，无需手动操作）
    - 如未登录，提示用户在自动打开的 Chrome 中登录（仅首次需要，后续自动保持登录态）
