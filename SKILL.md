...
根据用户使用的 Agent 平台，将 `RULES.md` 内容安装到对应位置：

| Agent 平台 | 配置方式 |
|-----------|---------|
| **Claude Code** | 复制为 `CLAUDE.md`；将 `agents/` 和 `skills/` 也复制一份到 `.claude/agents/` 和 `.claude/skills/` |
| **Cursor** | 复制为 `.cursor/rules/story-snack.md` |
| **Trae** | 复制为 `.trae/rules/story-snack.md` |
| **OpenClaw** | 复制为 `RULES.md`（已就位） |
| **其他** | 将 `RULES.md` 内容放入该 Agent 的项目级指令文件 |
